...
async fn test_external_dns_record_lifecycle() -> Result<()> {
    ...
    // We use a scopeguard pattern via a nested function to ensure cleanup.
    ...
    // Wait for the A record to appear, then also check for TXT ownership record.
    ...
    let wait_result = wait_for(Duration::from_secs(120), Duration::from_secs(5), || {
        ...
        async move {
            ...
            if !has_a {
                bail!("waiting for A record for {hostname}");
            }
            Ok(())
        }
    })
    ...
    let del_result = wait_for(Duration::from_secs(120), Duration::from_secs(5), || {
        ...
        async move {
            ...
            if still_exists {
                bail!("waiting for DNS records to be cleaned up for {hostname}");
            }
            Ok(())
        }
    })
