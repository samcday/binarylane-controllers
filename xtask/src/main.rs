...
enum Commands {
    ...
    Tilt(TiltArgs),
    /// Bump version in Cargo.toml, charts/binarylane-controllers/Chart.yaml, and Cargo.lock
    Bump {
        /// Version string (e.g. 1.2.3 or 1.2.3-rc.1)
        version: String,
    },
    /// Regenerate CRD YAML for the binarylane-controllers-crds Helm chart
    GenCrds,
}
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::GenCrds => cmd_gen_crds(),
    }
}

fn cmd_gen_crds() -> Result<()> {
    use binarylane_controller::crd::AutoScalingGroup;
    use kube::CustomResourceExt;

    let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap();
    let out_dir = workspace_root.join("charts/binarylane-controllers-crds/templates");
    fs::create_dir_all(&out_dir).context("creating CRDs templates dir")?;

    let mut crd = AutoScalingGroup::crd();
    // Without resource-policy: keep, `helm uninstall` would delete the CRD and
    // cascade-delete every AutoScalingGroup in the cluster.
    crd.metadata
        .annotations
        .get_or_insert_with(std::collections::BTreeMap::new)
        .insert("helm.sh/resource-policy".to_string(), "keep".to_string());

    let yaml = serde_yml::to_string(&crd).context("serializing CRD to YAML")?;
    let contents = format!("# Generated by 'cargo xtask gen-crds' — do not edit.\n{yaml}");

    let out = out_dir.join("autoscalinggroups.yaml");
...
fn cmd_bump(version: &str) -> Result<()> {
    ...
    let chart_yaml = workspace_root.join("charts/binarylane-controllers/Chart.yaml");
    ...
    // charts/binarylane-controllers/Chart.yaml: replace version and appVersion
    let contents = fs::read_to_string(&chart_yaml).context("reading Chart.yaml")?;
    let new_contents: String = contents
        .lines()
        .map(|line| {
    ...
    fs::write(&chart_yaml, &new_contents).context("writing Chart.yaml")?;
